...
1. `src/trie/base_storage/` — persistence abstraction for trie `Base`s. Traits `BaseStorageRead: Sync`/`BaseStorageReadWrite` (`read`/`max_id`/`read_root`/`get_root`; `next_id`/`append`/`write_root`/`to_readonly`). Implementations: `mem::MemBaseStorage` (a `Vec<Base>` seeded with the empty base at index 0) and `file::FileBaseStorage` (postcard-encoded base files in two-level subfolders under `<folder>/bases/`, with `max_id` and `root` files in the folder root).
...
3. `src/db/` — the Datomic layer over `Trie`: `Datom` (`ent`/`attr`/`dat`/`dir`), schema, and queries (`find`/`pull`). `Db<S>` wraps `schema` + `trie: Trie<S>`. `DbReader<S: BaseStorageRead>` is a read-only snapshot: `DbReader::load(db)` *borrows* a `Db<T>` and builds an independent snapshot reader over `T::ReadOnly` (bound via `T: BaseStorageReadWrite<ReadOnly = S>`, so `T` stays inferable from the `Db`); the `Db` remains usable, and writes after `load` are invisible to the reader. It implements `DbQuery`. The read-only query machinery (`db::find`, `find::program::{Program, KnowledgeBase, atom, rule}`, `db_trie::find/ev_stream`, `val_table::query`) is generic over `T: TrieQuery<S>, S: BaseStorageRead`, so it works for `Trie`, `TrieRef`, and `ReadTrie` alike; transitive helpers returning `TrieRef<'a, S>` need `S: 'a`. `DbHandle<S: BaseStorageReadWrite + Send + Sync + 'static>` (`db/handle.rs`) owns the `Db` in a worker task; `transact` sends datoms over an mpsc channel and `to_reader()` round-trips a `DbReader<S::ReadOnly>` snapshot through it.
