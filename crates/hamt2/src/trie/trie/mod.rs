...
impl<T: Space> SpaceTrie<T> {
    pub async fn connect(space: &T) -> Result<Self, QueryError> {
        ...
        let mut storage = SharedMemBaseStorage::new();
        let map_base = match reader.read_root().await? {
            ...
            Some(root) => {
                ...
                let trie_map_base = space_root.into_mem(&reader, &mut storage).await?;
                trie_map_base
            }
        };
        let trie = Self {
    ...
    pub async fn commit(self, space: &mut T) -> Result<(), TransactError> {
        ...
        let root_addr = {
            let space_map_base = self
                .map_base
                .into_space_map_base(&mut extend, &self.storage)
                .await?;
