...
#[derive(Debug)]
pub struct DbReader<S: ReadStorage + TrieBaseRead<Config = HandleTrieConfig>> {
    schema: Schema,
    read_trie: TrieReader<S>,
}

impl<S: ReadStorage + TrieBaseRead<Config = HandleTrieConfig>> DbReader<S> {
    ...
    where
        T: ReadWriteStorage<Snapshot = S> + TrieBaseRead<Config = HandleTrieConfig>,
    {
...
impl<S: ReadStorage + TrieBaseRead<Config = HandleTrieConfig>> DbQuery for DbReader<S> {
