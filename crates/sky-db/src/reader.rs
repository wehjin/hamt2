...
use sky_types::storage::{TrieView, TrieEdit};

/// A read-only snapshot of a [`Db`], for running queries only.
#[derive(Debug, Clone)]
pub struct DbReader<S: TrieView + Clone + Send> {
    schema: Schema,
    read_trie: TrieReader<S>,
}

impl<S: TrieView + Clone + Send> DbReader<S> {
    ...
    pub fn start(schema: Schema, storage: &impl TrieView<Snapshot = S>) -> Self {
        let read_trie: TrieReader<S> = TrieReader::<S>::new(storage.snapshot());
        DbReader { schema, read_trie }
    }
}

impl<S: TrieView + Clone + Send> DbQuery for DbReader<S> {
