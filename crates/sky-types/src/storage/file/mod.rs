...
impl FileReadStorage {
    ...
    fn read_base_unchecked(&self, id: SlotBaseId) -> Result<SlotBase, ReadStorageError> {
        if id == SlotBaseId::EMPTY {
            return Ok(SlotBase::empty());
        }
...
mod tests {
    ...
    use crate::trie::{HashKey, TrieRead, TrieValue};
