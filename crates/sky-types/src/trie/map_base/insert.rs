...
pub async fn insert_kv<P: BaseCommit>(
    ...
    let post_map_base = match map.try_base_index(key) {
        Some(base_index) => {
            ...
            match read_base.as_ref()[base_index].test_kv(&key, &value) {
                ...
                KvTest::ValueConflict => {
                    ...
                    let id = base_committer.commit_base(post_base).await?;
                    MapBase { map, base: id }
                }
                KvTest::KeyConflict => {
                    let post_base =
                        base::kick_kv(read_base, base_index, key, value, base_committer).await?;
                    let id = base_committer.commit_base(post_base).await?;
                    MapBase { map, base: id }
                }
        ...
        None => {
            ...
            let post_slot_base = {
                let base = base_committer.read_base(base_id).await?;
                let kv_index = map.count_left(key);
                base::insert_kv(base, kv_index, key, value)
            };
            let id = base_committer.commit_base(post_slot_base).await?;
