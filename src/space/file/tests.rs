...
async fn high_bits_work() {
    ...
    }
    {
        ...
        assert_eq!(
            SlotValue::from((0u32, 0u32)),
            reader.read_slot(&TableAddr::ZERO, 0).expect("read slot")
        );
        assert_eq!(
            SlotValue::from((0u32, 0x8000_0000u32)),
            reader.read_slot(&TableAddr::ZERO, 1).expect("read slot")
        );
        assert_eq!(
            SlotValue::from((0x8000_0000u32, 0u32)),
            reader.read_slot(&TableAddr::ZERO, 2).expect("read slot")
        );
        assert_eq!(
            SlotValue::from((0x8000_0000u32, 0x8000_0000u32)),
            reader.read_slot(&TableAddr::ZERO, 3).expect("read slot")
        );
    }
}
    ...
    }
    {
        ...
        for count in 1..=3 {
            for offset in 0..count {
                let slot = reader.read_slot(&addr, offset).expect("read slot");
                slots.push(slot);
            }
            addr += count;
        }
