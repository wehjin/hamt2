...
impl IrohKey {
    ...
    pub fn into_block_key(self) -> BlockKey {
        match self {
            IrohKey::Block(key) => key,
            _ => panic!("IrohKey is not a block key"),
        }
    }
    pub fn into_search_key(self) -> SearchKey {
        match self {
            IrohKey::Search(key) => key,
            _ => panic!("IrohKey is not a search key"),
        }
    }
}

impl Into<Bytes> for IrohKey {
    fn into(self) -> Bytes {
        match self {
            IrohKey::Details => Bytes::from_static(Self::PREFIX_DETAILS),
            IrohKey::Block(key) => {
...
mod tests {
    ...
    fn from_slice_for_block() {
        ...
        let iroh_key: IrohKey = bytes.as_ref().into();
        assert_eq!(
            iroh_key,
            IrohKey::Block(BlockKey::new(TableAddr::from(0x01020304u32), 0x05060708))
        );
    }

    #[test]
    fn into_bytes_for_details() {
        let iroh_key = IrohKey::Details;
        let bytes: Bytes = iroh_key.into();
    ...
    fn into_bytes_for_search() {
        let iroh_key = IrohKey::Search(SearchKey::from_addr(&TableAddr::from(0x01020304u32)));
        let bytes: Bytes = iroh_key.into();
